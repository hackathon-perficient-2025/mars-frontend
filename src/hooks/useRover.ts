    ...
    const updateRoverData = useCallback(() => {
        setRover((prev) => {
            ...
            return {
                ...
                batteryLevel: Math.max(0, Number((prev.batteryLevel - batteryDrain).toFixed(2))),
                lastCommunication: new Date(),
                // Simulate wheel wear slightly if moving
                wheels: prev.speed > 0 && Math.random() > 0.95 ? {
                    ...prev.wheels,
                    [Object.keys(prev.wheels)[Math.floor(Math.random() * 6)]]: Math.max(0, Object.values(prev.wheels)[Math.floor(Math.random() * 6)] - 0.1)
                } : prev.wheels
            };
        });
    }, []);
