...
export const RoverModel = ({ status, onPartClick }: RoverModelProps) => {
    ...
    const materialProps = (partName: string) => ({
        color: hoveredPart === partName ? '#ff6b6b' : '#e0e0e0',
        metalness: 0.6,
        roughness: 0.4,
    });
    ...
    const Wheel = ({ position, name }: { position: [number, number, number], name: string }) => (
        <mesh
            ...
            onPointerOver={(e) => { e.stopPropagation(); setHoveredPart(name); }}
    ...
    return (
        <group ref={groupRef}>
            ...
            <mesh
                position={[0, 0.8, 0]}
                onClick={(e) => { e.stopPropagation(); onPartClick('Main Body'); }}
                onPointerOver={(e) => { e.stopPropagation(); setHoveredPart('Main Body'); }}
                onPointerOut={() => setHoveredPart(null)}
            >
                <boxGeometry args={[1.8, 0.6, 2.4]} />
                <meshStandardMaterial {...materialProps('Main Body')} color={hoveredPart === 'Main Body' ? '#ff9999' : '#ffffff'} />
            </mesh>
            ...
            <group position={[0, 1.1, 0.8]}>
                ...
                <mesh
                    position={[0, 0.9, 0]}
                    onClick={(e) => { e.stopPropagation(); onPartClick('Camera Array'); }}
                    onPointerOver={(e) => { e.stopPropagation(); setHoveredPart('Camera Array'); }}
                    onPointerOut={() => setHoveredPart(null)}
                >
            ...
            <mesh
                ...
                onClick={(e) => { e.stopPropagation(); onPartClick('Power Source'); }}
                onPointerOver={(e) => { e.stopPropagation(); setHoveredPart('Power Source'); }}
                onPointerOut={() => setHoveredPart(null)}
            >
            ...
            {/* Wheels */}
            {/* Front */}
            <Wheel position={[-1.1, 0.4, 1.0]} name="Front Left Wheel" />
            <Wheel position={[1.1, 0.4, 1.0]} name="Front Right Wheel" />

            {/* Middle */}
            <Wheel position={[-1.1, 0.4, 0]} name="Middle Left Wheel" />
            <Wheel position={[1.1, 0.4, 0]} name="Middle Right Wheel" />

            {/* Rear */}
            <Wheel position={[-1.1, 0.4, -1.0]} name="Rear Left Wheel" />
            <Wheel position={[1.1, 0.4, -1.0]} name="Rear Right Wheel" />
